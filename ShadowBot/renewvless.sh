...
echo -e "\033[1;93m─────────────────────────────────────────\033[0m"
echo ""
grep -E "^#vle-user# " "/etc/xray/vle.json" | cut -d ' ' -f 2-3 | column -t | sort | uniq
echo ""
echo -e "\033[1;93m─────────────────────────────────────────\033[0m"
...
echo $iplim > /etc/lunatic/limit/vless/ip/${user}
if [ ! -e /etc/vless/ ]; then
  mkdir -p /etc/vless/
...
if [[ ${c} != "0" ]]; then
  echo "${d}" >/etc/lunatic/limit/vless/quota/${user}
    ...
    sed -i "/#vle-user# $user/c\#vle-user# $user $exp4" /etc/xray/vle.json
   # sed -i "/#! $user/c\#vle-user# $user $exp4" /root/akun/trojan/.trojan.conf
