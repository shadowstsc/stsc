...
echo -e "\033[1;93m─────────────────────────────────────────\033[0m"
echo ""
grep -E "^#tro-user# " "/etc/xray/tro.json" | cut -d ' ' -f 2-3 | column -t | sort | uniq
echo ""
echo -e "\033[1;93m─────────────────────────────────────────\033[0m"
...
echo $iplim > /etc/lunatic/limit/trojan/ip/${user}
...
if [[ ${c} != "0" ]]; then
  echo "${d}" >/etc/lunatic/limit/trojan/quota/${user}
    ...
    sed -i "/#tro-user# $user/c\#tro-user# $user $exp4" /etc/xray/tro.json
   # sed -i "/#tro-user# $user/c\#tro-user# $user $exp4" /root/akun/trojan/.trojan.conf
